...
def get_result(test_dir, lcs_mode):
    ...
    for i in seq_name_pairs:
        ...
        str1=i[0]
        mother_score[i[1]]=0;
        temp_timeOfCode[i[1]]=float(os.path.getmtime(test_dir+i[1]))
    ...
    for tup in scores:
        ...
        if temp_timeOfCode[name_map[name_a]] <= temp_timeOfCode[name_map[name_b]] :
            re_link={"source":name_map[name_a],"target":name_map[name_b],"weight":str_score}
        else :
            re_link={"source":name_map[name_b],"target":name_map[name_a],"weight":str_score}
    ...
    timeOfCode=sorted(temp_timeOfCode.items(), key=operator.itemgetter(1))

    for i in range(0,len(timeOfCode)) :
        re_node={"name":(timeOfCode[i][0])}
        nodes_array.append(re_node)
